...
export default function SequenceEditor({ sequence, features = [], enzymes = [], primers = [], initialCharsPerLine = 60 }) {
  ...
  // Enzyme label backgrounds + hit areas above primers (covers vertical lines cleanly)
    ...
    return enzymes.map(e => {
      ...
      return (
        <g key={e.id} onMouseEnter={() => setHoveredEnzyme(e.id)} onMouseLeave={() => setHoveredEnzyme(null)}>
          {/* Opaque background to mask enzyme vertical lines underneath */}
          <rect x={cutX + 3} y={yTop - 10} width={enzW + 2} height={18} fill={bgColor} />
          <text x={cutX + 6} y={yTop + 5} fill={color} fontSize="14px" fontFamily="Cascadia Code"
            fontWeight={e.isUnique ? '700' : '350'} style={{ pointerEvents: 'none' }}>
  ...
  const renderTooltips = () => {
    if (!hoveredEnzyme) return null;
    const e = enzymes.find(x => x.id === hoveredEnzyme);
    if (!e || e.recStart === undefined) return null;

    const recLen = e.recEnd - e.recStart + 1;
    // All X positions derived from absolute rec_start — no offset math
    const recStartCol = e.recStart % charsPerLine;
    const leftX = getX(recStartCol) - 8;
    const ttW = recLen * cw + 16;
    const sy = getSeqY(Math.floor(e.recStart / charsPerLine));
    const ttY = sy - 26;
    const sw = e.isUnique ? '2' : '1';
    const swM = e.isUnique ? '3' : '1.5';

    const cutX = getX(e.cutIndex % charsPerLine);
    const botCutX = e.botCutIndex != null ? getX(e.botCutIndex % charsPerLine) : cutX;

    const sub = cleanSeq.substring(e.recStart, e.recEnd + 1);
    const comp = sub.split('').map(complement).join('');
    const baseX = getX(recStartCol);
    const spacer = e.spacer;
    const isRec = spacer
      ? (i) => i < spacer.start || i >= spacer.end
      : () => true;

    return (
      <g key={`tt-${e.id}`} style={{ pointerEvents: 'none' }}>
        <rect x={leftX} y={ttY} width={ttW} height={60} rx={8} fill="#FFFFFF" stroke="#2563EB" strokeWidth={sw} />
        <line x1={cutX - 3} x2={cutX + 3} y1={ttY} y2={ttY} stroke="#FFFFFF" strokeWidth={swM} />
        ...
        <text y={sy} fontFamily={monoFont} fontSize="16px" fill="#1f2937">
          {sub.split('').map((c, i) => (
            <tspan key={i} x={baseX + i * cw + cw / 2} textAnchor="middle"
              fontWeight={isRec(i) ? '700' : '200'}>{c}</tspan>
          ))}
        </text>
        <text y={sy + 20} fontFamily={monoFont} fontSize="16px" fill="#6B7280">
          {comp.split('').map((c, i) => (
            <tspan key={i} x={baseX + i * cw + cw / 2} textAnchor="middle"
              fontWeight={isRec(i) ? '700' : '200'}>{c}</tspan>
          ))}
