...
- 序列编辑 → `edit_sequence`（`replacement` 字符串或 `replacement_path` 文件互斥，空串=删除；`expected_old` 乐观校验，失败返回差异索引与 ±20 bp 上下文；按 delta 平移/裁剪特征，恒返回 `removedFeatures`/`clippedFeatures` 回显副作用；protein 项目校验字母表 A-Z + 末尾可选 `*`）
...
- 序列比对 → `add_alignment`（`bases`/`path` 双输入，长读段用 `path`（.gbk/.dna/.fasta/.ab1）；返回 `identity`（全精度）、`alignedLength` 与差异明细 `mismatchDetails`/`deletionDetails`/`insertionDetails`；成功响应恒附全部已存比对的 `alignments` 数组；短读段最小 50 bp）
...
- `Feature.start/end`、`Feature.segments[]`、`PrimerBindingSite.template_start`、`BindingSite.matchStart/End` — 0-based inclusive
