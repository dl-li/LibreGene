...
- **工具**：22 个（`list_projects`、`get_project_overview`、`get_region_view`、`read_sequence`、`search_sequence`、`find_restriction_sites`、`list_primers`、`open_file`、`save_file`、`export_subsequence`、`close_project`、`activate_project`、`edit_sequence`、`add_feature`、`update_feature`、`add_primer`、`add_alignment`、`find_orfs`、`design_primers`、`check_primer_binding`、`optimize_cds`、`convert_coordinates`）。mutation 工具统一返回 `{ok, message, projectId, regionView?}`，regionView 为编辑后区域 digest 摘要。digest 中酶切列表只列单切酶、多切酶折叠计数；`get_region_view` 默认 compact（`compact: false` 得完整酶切列表），`get_project_overview` 的 UNIQUE CUTTERS 同理用 `compactCutters: false`。
...
- **测试**：`src-tauri` 内 `cargo test --lib` 覆盖 McpServer 启停/换端口、406/-32001 错误体、`check_primer_binding` 全位点与尾巴覆盖字段（`alignedTemplate`/`matchMask`）、`design_primers` amplify 的 `orientation`/`cdsOverlaps` 与 mutagenesis 整密码子替换警告分级、`design_primers` Tm 口径与 `check_primer_binding` 统一（`designedTm`/`designedAnnealLen` 保留原值）、`export_subsequence` 正反例、`add_feature`/`update_feature` 结构化 1-based 参数正反例、`add_alignment` 的 `orientedSequence`/`coverage`（正/反链、环状跨原点分段）与 compact 模式、`convert_coordinates` 三种互斥输入与负链分段 CDS 往返、`get_region_view` 差异节窗口过滤；digest 渲染（含 read_sequence 小窗口标尺紧凑化）与 `template_coverage`/mutagenesis 分析在 `libregene-core` 有单元测试，新增 `coords.rs` 坐标转换单元测试。
...
- 坐标转换 → `convert_coordinates`（三种互斥输入：① `position` 全文件 1-based 坐标；② `feature_id` + `feature_offset`（沿特征自身 5'→3' 方向的 1-based 偏移，负链特征从模板高位向低位计数）；③ `feature_id` + `aa_position`（CDS/mRNA 特征的 1-based 氨基酸位置）。返回 `position`、`base`（该位置的模板正链碱基，protein 项目为残基字母）、`features`（命中该位置的所有特征及其偏移/长度）、`translations`（CDS/mRNA 命中的密码子索引、双口径氨基酸编号 `aaPosition1Based`/`aaPositionExcludingMet`、编码链密码子、氨基酸、密码子第几碱基）；aa 输入额外返回 `codonPositions`（密码子三个 1-based 位置，按 5'→3' 生物学顺序）。a/b 对 DNA/RNA/protein 项目均可用；c 仅当存在 CDS/mRNA 命中时返回，不命中为空数组不报错）
...
- **Tm 参数与引物分析设置**：`design_primers` 已暴露 `na_conc`/`mg_conc`/`dntp_conc`/`tris_conc`/`primer_conc`；`check_primer_binding`/`add_primer` 仍用默认浓度。引物分析的可视化设置为渲染层状态
