...
└── src-tauri/                  # Tauri v2 桌面壳
    ...
    └── src/
        ├── lib.rs              # Tauri commands + AppState + 共享 do_* 内核 + OS 文件打开事件（Opened/argv/单实例转发 → pending_opens 队列 + file-opened 事件）+ 系统托盘（close-to-tray：关主窗口仅隐藏，托盘菜单 MCP 状态/Show/Quit，macOS Dock Reopen 重开窗口）
...
- **后台待命（close-to-tray）**：关闭主窗口只是隐藏窗口（`on_window_event` 拦截 `CloseRequested`），进程与 MCP server 继续运行，Agent 可随时调用；系统托盘图标常驻，菜单含 MCP 状态行（`set_mcp_config` 改配置时同步刷新文字）、Show LibreGene（左键点托盘图标同效；macOS 点 Dock 图标经 `RunEvent::Reopen` 重开）、Quit LibreGene（与 Cmd+Q 等效，彻底退出）。项目窗口（`project-*`）不参与，仍正常关闭。
