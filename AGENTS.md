...
- 项目/文件管理（打开/保存/关闭/切换）→ `open_file`、`save_file`、`close_project`、`activate_project`、`list_projects`（`open_file` 支持 `.rna`/`.prot`/`.gpt` 及 GenPept 变体 `.gp`/`.gpe`/`.gpff`、GenBank 变体 `.gbf`/`.gbff`、FASTA 变体 `.fas`/`.ffn`/`.fsa`/`.frn`（`.faa` 按蛋白打开）、内容嗅探的 `.seq`；`save_file` 按扩展名写 `.gbk`/`.gb`/`.gpt`；`list_projects` 返回 `moleculeType`；命名语义：`open_file` 打开文件成为项目并返回 `projectId`，之后所有工具用 `project_id` 引用该项目，list/activate/close 操作的是内存中的项目而非磁盘文件）
...
- 序列比对（Sanger reads / 序列）→ `add_alignment`（`bases`/`path` 双输入，长读段建议 `path`；`path` 支持 .gbk/.dna/.fasta/.ab1；返回差异明细：`mismatchDetails` 每个 mismatch 的 0-based 模板位置与模板/读段碱基、`deletionDetails` 每个 deletion 的位置/长度/缺失碱基（跨环状原点自动合并）、`insertionDetails` 每个 insertion 的插入位点（pos-1 与 pos 之间）与序列/长度；另有 `identity`（全精度不四舍五入）与 `alignedLength`（覆盖模板长度）；成功响应恒附带 `alignments` 数组——该项目当前全部比对（每个含 alignmentId/name/identity/strand/segmentCount/alignedLength/mismatches/insertions/deletions 及差异明细），无需单独只读工具即可查看所有已存比对；短读段被拒时错误信息说明原因与最小长度阈值 50 bp）
- IUPAC 序列搜索 → `search_sequence`
- 甲基化设置 → `set_methylation`
- 限制酶切位点查询 → `find_restriction_sites`（按名称列出识别位点与切口：`recStart`/`recEnd` 0-based inclusive、`recSeq`、识别链 `top`/`bottom`、`cuts` 每个 `topCutIndex`/`botCutIndex`（切口在 cut-1 与 cut 之间）；复用已算好的引擎结果，环状坐标已归一化，不传 `enzymes` 返回全部，未知酶名报错并给出近似名——用这个报错探测本序列上有切口的酶名，替代已删除的整库 `get_enzyme_database` 工具）
...
- 密码子优化（CDS 同义替换 + CAI 对比）→ `optimize_cds`（三种互斥输入：① `project_id`+`feature_id` 项目内特征（project_id 缺省=活动项目），② `sequence` 直接传 DNA 编码序列（容忍空格/换行/数字，校验 ACGT、长度 %3，可带结尾终止密码子），③ `input_path` 本地文件（DNA 文件 .gbk/.gb/.genbank/.dna/.rna/.fasta/.fa/.fna/.ab1 走 file_io::parse_file，带 feature_id 用文件内 CDS/mRNA 特征、否则整段序列当 CDS；蛋白文件 .gpt/.prot = 反向翻译：aa 直接按目标物种表生成优化 DNA，引擎在 core `optimize_from_aa`）；apply=false 预览返回 AA/新密码子/前后 CAI/GC/repairs（repairCount 计数）/unresolved，只读不落库；sequence/input_path 模式额外返回 `optimizedSequence`（完整优化 DNA，特征模式为整段序列写回结果）；`output_path` 可选写结果文件（.gbk/.gb/.genbank → DNA GenBank 含优化 CDS 特征，.gpt → 翻译后蛋白 GenBank，未知扩展名报错，返回 `outputPath`）；apply=true 仅 project 模式有意义（按 segments_on_template 等长写回模板序列并广播，返回 regionView），sequence/input_path 模式下 apply=true 无 output_path 报错；method 为 use_best_codon（默认）/match_codon_usage/harmonize_rca，harmonize_rca 需 original_species 源表；species 用 `list_species` 键名；Tauri 侧另有 `preview_codon_optimization`/`apply_codon_optimization`（带 customTable/gcWindow/avoidEnzymeSites 扩展参数）与 `list_codon_species`）
