...
import {
  computePrimerSegments,
  mismatchOffset,
  buildSegmentPath,
  buildSegmentHoverPath,
} from './primerRenderer';
...
const STYLES = {
  default: {
    ...
    expandedLabelColor: () => bgColor,
    expandedLabelStroke: (pColor) => pColor,
  },
  selected: {
    ...
    labelColor: (pColor) => pColor,
    labelStroke: () => bgColor,
    showLabelCollapsed: false,
    showLabelExpanded: true,
    expandedLabelColor: (pColor) => pColor,
    expandedLabelStroke: () => bgColor,
  },
  hover: {
    bg: () => ({ fill: bgColor }),
...
export default function PrimerSegmentRenderer({
  ...
  const getRowYs = (row) => {
    ...
    return (rowCache[row] = {
      ...
      mismatchY:
        base + (isFwd ? -trackOff : trackOff) + extraR + (isFwd ? -mismatchOffset : mismatchOffset),
    ...
    <g>
      ...
      {hasFivePrimeTail &&
        (() => {
          ...
          const edge5x = isFwd ? getX(fivePrimeSeg.colStart) : getX(fivePrimeSeg.colEnd) + cw;
          ...
          return (
            <g key="5tail">
              {/* Tail line */}
              <path
                d={`M ${tailStartX} ${rowYs.misY} L ${tailEndX} ${rowYs.misY}`}
                fill="none"
                stroke={bgColor}
                strokeWidth="5"
                strokeLinecap="round"
              ...
              {/* Invisible hit area */}
              <line
                x1={Math.min(tailStartX, tailEndX)}
                y1={rowYs.misY}
                x2={Math.max(tailStartX, tailEndX)}
                y2={rowYs.misY}
                stroke="transparent"
                strokeWidth="20"
              />
      ...
      {segs.map((seg) => {
        ...
        const pathData = buildSegmentPath(
          ...
          charsPerLine,
          isThreePrime,
          isThreePrime && hasThreePrimeTail,
          threePrimeTail,
          arrowWidth,
          arrowHeight,
        );
        ...
        const hoverPath = buildSegmentHoverPath(
          ...
          isThreePrime && hasThreePrimeTail,
          threePrimeTail,
          seg === fivePrimeSeg && hasFivePrimeTail,
          fivePrimeTail,
        );
        ...
        return (
          <g key={`${seg.row}-${seg.colStart}`}>
            ...
            {style.overlay(pColor) && hoverPath && (
              <path d={hoverPath} {...style.overlay(pColor)} />
            )}
                ...
                return (
                  <g key={insKey}>
                    ...
                    <line
                      ...
                      stroke="transparent"
                      strokeWidth="12"
                      onMouseEnter={() => handleInsEnter(insKey)}
                      onMouseLeave={handleInsLeave}
                    />
                    {hoveredInsertion === insKey && showText && (
                      <g style={{ pointerEvents: 'none' }}>
                        <rect
                          x={insX - (dispBases.length * cw) / 2 - 4}
                          y={isFwd ? insTop - 22 : insBot + 2}
                          width={dispBases.length * cw + 8}
                          height={20}
                          rx={4}
                          fill={bgColor}
                          stroke={pColor}
            ...
            {style.showLabelCollapsed && seg === fivePrimeSeg && (
              <text
                x={getLabelX(seg)}
                y={getRowYs(seg.row).matchY + textYLabel}
                fill={style.labelColor(pColor)}
                fontSize="12px"
                fontFamily="TeX Gyre Heros"
                fontWeight="600"
                fontStyle="italic"
                textAnchor={isFwd ? 'start' : 'end'}
                stroke={style.labelStroke(pColor)}
                strokeWidth="4"
            ...
            {style.showLabelExpanded && (
              <text
                x={getLabelX(seg)}
                y={getRowYs(seg.row).matchY + (isFwd ? textYLabel - 15 : textYLabel + 12)}
                fill={style.expandedLabelColor(pColor)}
                fontSize="12px"
                fontFamily="TeX Gyre Heros"
                fontWeight="600"
                fontStyle="italic"
                textAnchor={isFwd ? 'start' : 'end'}
                stroke={style.expandedLabelStroke(pColor)}
                strokeWidth="4"
                paintOrder="stroke fill"
                style={{ opacity: 1, transition: springAnim, pointerEvents: 'none' }}
              >
