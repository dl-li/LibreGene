...
function NumInput({ label, value, onChange, min, max, step }) {
  return (
    <div className="flex items-center justify-between gap-2">
      <Label
        className="text-xs font-normal text-muted-foreground w-36 shrink-0 truncate"
        title={label}
      >
        {label}
      </Label>
      <Input
        className="w-16 h-7 px-1.5 py-0 text-xs text-right font-mono"
        type="number"
        value={value}
        min={min}
        max={max}
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="dp-features" className="cursor-pointer text-xs">
                特征 ({features.length})
              </Label>
            </div>
            <div className="flex items-center gap-2">
              ...
              />
              <Label htmlFor="dp-primers" className="cursor-pointer text-xs">
                引物 ({primers.length})
              </Label>
            </div>
          </div>
          ...
          <div>
            ...
            {!isTauri && (
              <>
                <Input
                  className="w-full text-[11px] h-7 mt-1.5 font-mono"
                  type="text"
                  value={openPath}
            ...
            {fileStatus && (
              <div className="text-[11px] mt-1 text-muted-foreground">{fileStatus}</div>
            )}
          </div>
          ...
          <CollapsibleSection title="行距">
            <NumInput
              label="最小行间距 (px)"
              value={lp.minRowGap}
              onChange={(v) => setLP('minRowGap', v)}
              min={8}
              max={80}
            />
            <NumInput
              label="行内容间距 (px)"
              value={lp.rowContentGap}
              onChange={(v) => setLP('rowContentGap', v)}
              min={0}
              max={40}
            />
            <NumInput
              label="上方交互区 (px)"
              value={lp.minAboveSpace}
              onChange={(v) => setLP('minAboveSpace', v)}
              min={8}
              max={60}
            />
            <NumInput
              label="下方交互区 (px)"
              value={lp.minBelowSpace}
              onChange={(v) => setLP('minBelowSpace', v)}
              min={0}
              max={40}
            />
          </CollapsibleSection>
          ...
          <CollapsibleSection title="特征排版">
            <NumInput
              label="特征行高 (px)"
              value={lp.featTrackHeight}
              onChange={(v) => setLP('featTrackHeight', v)}
              min={6}
              max={40}
            />
            <NumInput
              label="特征距序列 (px)"
              value={lp.featBaseOffset}
              onChange={(v) => setLP('featBaseOffset', v)}
              min={4}
              max={40}
            />
            <NumInput
              label="特征标签内边距 (px)"
              value={lp.featLabelPad}
              onChange={(v) => setLP('featLabelPad', v)}
              min={0}
              max={30}
            />
          </CollapsibleSection>
          ...
          <CollapsibleSection title="酶切位点排版">
            <NumInput
              label="酶标签行高 (px)"
              value={lp.enzTrackHeight}
              onChange={(v) => setLP('enzTrackHeight', v)}
              min={6}
              max={40}
            />
            <NumInput
              label="酶连线到序列 (px)"
              value={lp.enzLineGap}
              onChange={(v) => setLP('enzLineGap', v)}
              min={4}
              max={40}
            />
            <NumInput
              label="酶标签基位 (px)"
              value={lp.enzLabelBase}
              onChange={(v) => setLP('enzLabelBase', v)}
              min={20}
              max={100}
            />
            <NumInput
              label="酶区域上边距 (px)"
              value={lp.enzAbovePad}
              onChange={(v) => setLP('enzAbovePad', v)}
              min={0}
              max={30}
            />
          </CollapsibleSection>
          ...
          <CollapsibleSection title="F 引物排版">
            <NumInput
              label="匹配线距序列 (px)"
              value={lp.fwdMatchY}
              onChange={(v) => setLP('fwdMatchY', v)}
              min={0}
              max={100}
            />
            <NumInput
              label="碱基文字 Y (px)"
              value={lp.fwdBaseTextY}
              onChange={(v) => setLP('fwdBaseTextY', v)}
              min={0}
              max={40}
            />
            <NumInput
              label="名称标签 Y (px)"
              value={lp.fwdLabelY}
              onChange={(v) => setLP('fwdLabelY', v)}
              min={0}
              max={40}
            />
            <NumInput
              label="上方基础间距 (px)"
              value={lp.fwdAboveBase}
              onChange={(v) => setLP('fwdAboveBase', v)}
              min={0}
              max={80}
            />
            <NumInput
              label="尾部附加间距 (px)"
              value={lp.fwdAboveExtra}
              onChange={(v) => setLP('fwdAboveExtra', v)}
              min={0}
              max={60}
            />
            <NumInput
              label="无尾部间距 (px)"
              value={lp.fwdAboveNonTailExtra}
              onChange={(v) => setLP('fwdAboveNonTailExtra', v)}
              min={0}
              max={30}
            />
          </CollapsibleSection>
          ...
          <CollapsibleSection title="R 引物排版">
            <NumInput
              label="匹配线距序列 (px)"
              value={lp.revMatchY}
              onChange={(v) => setLP('revMatchY', v)}
              min={0}
              max={100}
            />
            <NumInput
              label="碱基文字 Y (px)"
              value={lp.revBaseTextY}
              onChange={(v) => setLP('revBaseTextY', v)}
              min={0}
              max={40}
            />
            <NumInput
              label="名称标签 Y (px)"
              value={lp.revLabelY}
              onChange={(v) => setLP('revLabelY', v)}
              min={0}
              max={40}
            />
            <NumInput
              label="下方基础间距 (px)"
              value={lp.revBelowBase}
              onChange={(v) => setLP('revBelowBase', v)}
              min={0}
              max={80}
            />
            <NumInput
              label="尾部附加间距 (px)"
              value={lp.revBelowExtra}
              onChange={(v) => setLP('revBelowExtra', v)}
              min={0}
              max={60}
            />
            <NumInput
              label="无尾部间距 (px)"
              value={lp.revBelowNonTailExtra}
              onChange={(v) => setLP('revBelowNonTailExtra', v)}
              min={0}
              max={30}
            />
          </CollapsibleSection>
          ...
          <CollapsibleSection title="引物通用">
            ...
            <NumInput
              label="错配文字 Y 偏移 (px)"
              value={lp.misYDelta}
              onChange={(v) => setLP('misYDelta', v)}
              min={0}
              max={20}
            />
            <NumInput
              label="悬停展开 (px)"
              value={lp.hoverExpand}
              onChange={(v) => setLP('hoverExpand', v)}
              min={0}
              max={60}
            />
            <NumInput
              label="箭头长度 (px)"
              value={lp.arrowHeadLen}
              onChange={(v) => setLP('arrowHeadLen', v)}
              min={0}
              max={20}
            />
            <NumInput
              label="箭头高度 (px)"
              value={lp.arrowHeadHeight}
              onChange={(v) => setLP('arrowHeadHeight', v)}
              min={0}
              max={20}
            />
          </CollapsibleSection>
