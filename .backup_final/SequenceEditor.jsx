...
const enzLabelW = (name, isUnique) => measureWidth(name, `${isUnique ? 'bold ' : ''}14px TeX Gyre Termes`) + 4;
...
export default function SequenceEditor({ sequence, features = [], enzymes = [], primers = [], initialCharsPerLine = 60 }) {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (containerRef.current) {
        setCharsPerLine(Math.max(20, Math.floor((containerRef.current.clientWidth - 300) / cw)));
      }
    };
    handleResize();
  ...
  const svgWidth = startX + charsPerLine * cw + 150;
  ...
  for (let r = 0; r < numRows - 1; r++) {
    rowY.push(rowY[r] + Math.max(rowAbove[r] + rowBelow[r] + 24, rowBelow[r] + rowAbove[r + 1] + 14));
  }
  const getSeqY = (row) => rowY[Math.min(row, rowY.length - 1)];
  const svgHeight = rowY[rowY.length - 1] + rowAbove[numRows - 1] + rowBelow[numRows - 1] + 74;
  ...
  const enzymeTracks = useMemo(() => {
    ...
    for (const e of sorted) {
      ...
      const ce = cs + Math.ceil(measureWidth(e.name, `normal 14px TeX Gyre Termes`) / cw) + 1;
      let t = 0;
        ...
        <g key={`${p.id}-${idx}`}>
          {segs.map(seg => {
            ...
            if (isTail && hasMis && drawMisLen > 0) {
              ...
              else pts.push([getX(seg.colEnd + drawMisLen), misY], [x2 + cw * 1.5, misY]);
            }
            if (isFwd) pts.push([x1 + cw / 2, matchY], [x2 + cw, matchY]);
            else pts.push([x2 + cw / 2, matchY], [x1, matchY]);
  ...
  const renderEnzymes = () => {
    const sorted = [...enzymes].sort((a, b) => {
      const ha = hoveredEnzyme === a.id, hb = hoveredEnzyme === b.id;
      if (ha !== hb) return ha ? 1 : -1;
      return (enzymeTracks[a.id] || 0) - (enzymeTracks[b.id] || 0);
    });

    return sorted.map(e => {
      const isHovered = hoveredEnzyme === e.id;
      const row = Math.floor(e.cutIndex / charsPerLine);
      ...
      const yTop = hy - 25 - to;
      const yBottom = isHovered ? sy - 25 : sy - 18;
      const color = isHovered ? '#2563EB' : '#333';

      return (
        <g key={e.id} onMouseEnter={() => setHoveredEnzyme(e.id)} onMouseLeave={() => setHoveredEnzyme(null)}>
          {isHovered && <line x1={cutX} x2={cutX} y1={yTop} y2={yBottom} stroke={bgColor} strokeWidth="6" strokeLinecap="square" />}
          <line x1={cutX} x2={cutX} y1={yTop} y2={yBottom} stroke={color} strokeWidth={e.isUnique ? (isHovered ? '2' : '1') : '0.8'} style={{ pointerEvents: 'none' }} />
          <rect x={cutX + 4} y={yTop - 10} width={enzLabelW(e.name, e.isUnique)} height={18} fill="transparent"
            className="cursor-pointer" style={{ pointerEvents: 'none' }} />
          <text x={cutX + 6} y={yTop + 5} fill={color} fontSize="14px" fontFamily="TeX Gyre Termes"
            fontWeight={e.isUnique ? 'bold' : 'normal'}>
  ...
  const renderEnzymeOverlay = () => {
    ...
    return (
      <g style={{ pointerEvents: 'none' }}>
        <line x1={cutX} x2={cutX} y1={yTop} y2={sy - 25} stroke={bgColor} strokeWidth="6" strokeLinecap="square" />
        <line x1={cutX} x2={cutX} y1={yTop} y2={sy - 25} stroke="#2563EB" strokeWidth={e.isUnique ? '2' : '1'} />
        <text x={cutX + 6} y={yTop + 5} fill="#2563EB" fontSize="14px" fontFamily="TeX Gyre Termes"
          fontWeight={e.isUnique ? 'bold' : 'normal'}>
            {(() => { const s = splitEnzName(e.name); return s.normal ? [<tspan key="i" fontStyle="italic">{s.italic}</tspan>, <tspan key="n">{s.normal}</tspan>] : e.name; })()}
          </text>
      </g>
  ...
  const renderTooltips = () => {
    ...
    const cutX = getX(e.cutIndex % charsPerLine);
    const sy = getSeqY(row);
    const len = e.seq.length;
    const sw = e.isUnique ? '2' : '1';
    const swM = e.isUnique ? '3' : '1.5';

    if (e.recStart === undefined) {
      const leftX = cutX - e.topCut * cw - 8;
      const ttW = len * cw + 16;
      const ttY = sy - 26;
      const botCutX = cutX + (e.botCut - e.topCut) * cw;
      const si = e.cutIndex - e.topCut;
      const sub = Array.from({ length: len }, (_, i) => cleanSeq[si + i] || 'N').join('');
      const comp = sub.split('').map(complement).join('');
      const spacer = e.spacer; // { start, end } — relative indices within seq that are non-specific

      if (spacer) {
        const spLeft = leftX + 8 + spacer.start * cw;
        const spRight = leftX + 8 + spacer.end * cw;
        const cutInRec = (cutX >= spRight) ? '#dbeafe' : '#FFFFFF';
        return (
          <g key={`tt-${e.id}`} style={{ pointerEvents: 'none' }}>
            <rect x={leftX} y={ttY} width={spacer.start * cw + 8} height={60} fill="#dbeafe" />
            <rect x={spLeft} y={ttY} width={(spacer.end - spacer.start) * cw} height={60} fill="#FFFFFF" />
            <rect x={spRight} y={ttY} width={ttW - (spacer.end * cw + 8)} height={60} fill="#dbeafe" />
            <rect x={leftX} y={ttY} width={ttW} height={60} rx={8} fill="none" stroke="#2563EB" strokeWidth={sw} />
            <line x1={cutX - 3} x2={cutX + 3} y1={ttY} y2={ttY} stroke={cutInRec} strokeWidth={swM} />
            <path d={`M ${cutX} ${ttY - 2} L ${cutX} ${sy + 3} L ${botCutX} ${sy + 3} L ${botCutX} ${sy + 28}`}
              fill="none" stroke="#2563EB" strokeWidth={sw} strokeLinejoin="round" />
            <text y={sy} fontFamily={monoFont} fontSize="18px" fontWeight="bold" fill="#1f2937">
              {sub.split('').map((c, i) => <tspan key={`t-${i}`} x={cutX - e.topCut * cw + i * cw + cw / 2} textAnchor="middle">{c}</tspan>)}
            </text>
            <text y={sy + 20} fontFamily={monoFont} fontSize="18px" fontWeight="bold" fill="#6B7280">
              {comp.split('').map((c, i) => <tspan key={`b-${i}`} x={cutX - e.topCut * cw + i * cw + cw / 2} textAnchor="middle">{c}</tspan>)}
            </text>
          </g>
        );
      }
      ...
      return (
        <g key={`tt-${e.id}`} style={{ pointerEvents: 'none' }}>
          <rect x={leftX} y={ttY} width={ttW} height={60} rx={8} fill="#dbeafe" stroke="#2563EB" strokeWidth={sw} />
          <line x1={cutX - 3} x2={cutX + 3} y1={ttY} y2={ttY} stroke="#dbeafe" strokeWidth={swM} />
          <path d={`M ${cutX} ${ttY - 2} L ${cutX} ${sy + 3} L ${botCutX} ${sy + 3} L ${botCutX} ${sy + 28}`}
            fill="none" stroke="#2563EB" strokeWidth={sw} strokeLinejoin="round" />
          <text y={sy} fontFamily={monoFont} fontSize="18px" fontWeight="bold" fill="#1f2937">
            {sub.split('').map((c, i) => <tspan key={`t-${i}`} x={cutX - e.topCut * cw + i * cw + cw / 2} textAnchor="middle">{c}</tspan>)}
    ...
    // 外切酶
    const recStart = e.recStart, recEnd = recStart + len;
    const botCutIdx = e.botCutIndex ?? (e.cutIndex + e.botCut - e.topCut);
    let minIdx = Math.min(recStart, e.cutIndex, botCutIdx);
    let maxIdx = Math.max(recEnd, e.cutIndex, botCutIdx);
    if (minIdx < recStart) minIdx--;
    if (maxIdx > recEnd) maxIdx++;
    const totalLen = maxIdx - minIdx;
    const minOff = minIdx - e.cutIndex;
    const leftX = cutX + minOff * cw - 8;
    const ttW = totalLen * cw + 16;
    const ttY = sy - 26;
    const recOff = recStart - e.cutIndex;
    const recSX = cutX + recOff * cw;
    const recW = len * cw;
    const botCutX = cutX + (botCutIdx - e.cutIndex) * cw;
    const sub = Array.from({ length: totalLen }, (_, i) => cleanSeq[minIdx + i] || 'N').join('');
    const comp = sub.split('').map(complement).join('');
    const adjRX = minIdx === recStart ? leftX : recSX;
    const adjRW = (maxIdx === recEnd ? leftX + ttW : recSX + recW) - adjRX;

    return (
      <g key={`tt-${e.id}`} style={{ pointerEvents: 'none' }}>
        <defs>
          <clipPath id={`clip-tt-${e.id}`}>
            <rect x={leftX} y={ttY} width={ttW} height={60} rx={8} />
          </clipPath>
        </defs>
        <rect x={leftX} y={ttY} width={ttW} height={60} fill="#FFFFFF" clipPath={`url(#clip-tt-${e.id})`} />
        <rect x={adjRX} y={ttY} width={adjRW} height={60} fill="#dbeafe" clipPath={`url(#clip-tt-${e.id})`} />
        <rect x={leftX} y={ttY} width={ttW} height={60} rx={8} fill="none" stroke="#2563EB" strokeWidth={sw} />
        <line x1={cutX - 3} x2={cutX + 3} y1={ttY} y2={ttY}
          stroke={recSX <= cutX && cutX <= recSX + recW ? '#dbeafe' : '#FFFFFF'} strokeWidth={swM} />
        <path d={`M ${cutX} ${ttY - 2} L ${cutX} ${sy + 3} L ${botCutX} ${sy + 3} L ${botCutX} ${sy + 28}`}
          fill="none" stroke="#2563EB" strokeWidth={sw} strokeLinejoin="round" />
        <text y={sy} fontFamily={monoFont} fontSize="18px" fontWeight="bold" fill="#1f2937">
          {sub.split('').map((c, i) => <tspan key={`t-${i}`} x={cutX + minOff * cw + i * cw + cw / 2} textAnchor="middle">{c}</tspan>)}
        </text>
        <text y={sy + 20} fontFamily={monoFont} fontSize="18px" fontWeight="bold" fill="#6B7280">
          {comp.split('').map((c, i) => <tspan key={`b-${i}`} x={cutX + minOff * cw + i * cw + cw / 2} textAnchor="middle">{c}</tspan>)}
        </text>
      </g>
    );
  };
  ...
  const renderSeq = () => (
    Array.from({ length: numRows }, (_, r) => {
      ...
      return (
        <text key={r} y={getSeqY(r)} fontFamily={monoFont} fontSize="18px" fontWeight="bold" fill="#1f2937"
  ...
  return (
    <div ref={containerRef} style={{ backgroundColor: bgColor, width: '100%', minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', paddingTop: '2rem', paddingLeft: '2rem', paddingBottom: '4rem', overflowX: 'hidden', userSelect: 'none' }} className="select-none">
